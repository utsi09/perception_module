...
# 중요: Translation Vector(4열)는 -R * (Cam_pos - Lidar_pos) 로 계산되어야 합니다.
        
        # 1. Front Camera (X=2.0, Z=2.0) -> Lidar(Z=2.4)보다 2m 앞, 0.4m 아래
        # Lidar는 카메라 기준: 뒤로 2m(-Z), 위로 0.4m(-Y)에 위치
        ...
        # 2. Left Camera (Y=1.0, Z=1.0) -> Lidar보다 왼쪽(Y+)으로 1m, 아래로 1.4m
        # Lidar는 카메라 기준: 오른쪽(카메라 X축 아님, vehicle X축인 X_cam), 뒤로(-Z), 위로(-Y)
        # 계산: t = -R * (P_cam - P_lidar)
        ...
        # 3. Right Camera (Y=-1.0, Z=1.0) -> Lidar보다 오른쪽(Y-)으로 1m, 아래로 1.4m
        ...
        # 4. Back Camera (X=-2.0, Z=2.0) -> Lidar보다 2m 뒤, 0.4m 아래
        # Lidar는 카메라 기준: 앞으로 2m(Z가 아니라 -Z방향.. 즉 뒤집힘), 위로 0.4m
        ...
        self.obj_pc_pub = self.create_publisher(PointCloud2, "/fusion/object_points", 10)
        self.overlay_pubs = {
    ...
    def sync_callback(self, lidar_msg, front_msg, left_msg, right_msg, back_msg):
        ...
        for i, cam_name in enumerate(cam_names):
            ...
            for obj_idx in range(num_obj):
                ...
                #### 11월 22일 수정 ### 
                """
                카메라 좌표계 포인트들을 라이다 좌표계로 변환해서 pc2로 발행
                """
                if i==0:
                    ...
                    pts_obj_lidar = cam2lidar @ pts_obj_h
                    pts_obj_lidar = pts_obj_lidar[:3,:].T
                    self.get_logger().info(f'{pts_obj_lidar}')

                    pc_msg = pc2.create_cloud_xyz32(pc_header, pts_obj_lidar.tolist())
                    self.obj_pc_pub.publish(pc_msg)
                ######################
